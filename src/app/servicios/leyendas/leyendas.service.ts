...
constructor(
  ...
  getAll() {
    let url = `${environment.urlApi}/leyenda/consultaGeneral`;
  ...
  get(id: string){
    let url = `${environment.urlApi}/leyenda/consultaId/${id}`;
    return this.httpClient.get<HttpResponse<any>>(url).pipe(
      map((m: any) => {
        if (m != undefined){          
          return m;
  ...
  save(form: any){    
    let url = `${environment.urlApi}/leyenda/alta`;       
  ...
  update(form: any) {
    let url = `${environment.urlApi}/leyenda/update/${form.idleyenda}`;
