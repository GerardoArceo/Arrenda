...
constructor(
  ...
  getAll() {
    let url = `${environment.urlApi}/monedas/consultaGeneral`;
  ...
  get(id: string){
    let url = `${environment.urlApi}/monedas/consultaId/${id}`;
    return this.httpClient.get<HttpResponse<any>>(url).pipe(
      map((m: any) => {
        if (m != undefined){          
          return m;
  ...
  save(form: any){    
    let url = `${environment.urlApi}/monedas/alta`;       
  ...
  update(form: any) {
    let url = `${environment.urlApi}/monedas/update/${form.cvemoneda}`;
